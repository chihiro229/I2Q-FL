...
class FedData(metaclass=ABCMeta):
    ...
    def _load_and_process_data(self):
        ...
        if ConfigurationManager().model_config.ml_method_name == 'CNN':
    ...
    def _save_dataset_files(self, dataset: List[Mapping[str, List[np.ndarray]]]) -> None:
        ...
        for i in range(len(dataset)):
            ...
            # comment the code to get equal number of samples for all clients
            # making a arbitrary random selection *************************************************
            train = random.sample(train, random.choice(range(1,len(train)+1)))
            val = random.sample(val, random.choice(range(1,len(val)+1)))
            test = random.sample(test, random.choice(range(1,len(test)+1)))
...
#                label_counts = dict()
#                for i in range(19):
#                    label_counts[str(i)] = 0
#                for x in self.y[train]:
#                    p = np.array(x)
#                    label_counts[str(p.argmax())] += 1
...
#                print(' ')
           ...
           # checking_assigned_label
            ...
            #rchecking_assigned_label.append(list(set(
            hickle.dump(target, os.path.join(
                self.output_dir, f'client_{i}.pkl'))
            del target

        print('*'*80)
        print(f'client_data_size : {client_data_size}')
        print('*'*80)
        ...
        if strategy == 'natural':
            ...
            if self.identity is not None:
                raise AttributeError(
                    'Selected dataset has identity, Please set Non-IID strategy to "natural" !')
            ...
            # gtsrb changes*******************************************************************
            # comment code to use dirichlet distribution

            # if True :
            #     for i in range(self.num_clients) :
            #         local_dataset.append([
            #         train_index[i * train_size : (i + 1) * train_size],
            #         val_index[i * val_size: (i + 1) * val_size],
            ...
            # gtsrb changes for dirichlet distribution from whole dataset to x number of samples per client
            # *********************************************************************************************
            if d_cfg.dataset_name == 'mnistt':
           # if True :                
                # we are going to divide entire data to clients

                revy = np.argmax(self.y, axis=1)
                N = len(self.x)  # number of samples

                min_size = 0 # number of samples per client required, Must always be min_require_size * n_parties <= N

                while min_size < sample_size:
                    idx_batch = [[] for _ in range(self.num_clients)]
                    for k in range(self.num_class):
                        # idx_k = np.where(y == k)[0]
                        # i think it is trying to find those indices of the dataset that belong to the class k
                        # we can perhaps choose to avoid shuffling the original data but makes not much difference since we
                        # must iterate regardless

                        idx_k = [idx for idx in range(len(revy)) if k == revy[idx]]
                        np.random.shuffle(idx_k)

                        proportions = np.random.dirichlet(np.repeat(0.1, self.num_clients))
                ...
                # and then divide that per percentage to train, val, test

                for j in range(self.num_clients):
                    np.random.shuffle(idx_batch[j])
                    # num = len(idx_batch[j]) # may be greater than assigned sample_size , we chose to use exactly sample_size only
                    train_index = train_size
                    val_index = train_size + val_size
                    test_index = train_size + val_size + test_size

                    train_data = idx_batch[j][:train_index]
                    val_data = idx_batch[j][train_index : val_index]
                    test_data = idx_batch[j][val_index : test_index]              

                    local_dataset.append([train_data, val_data, test_data])
