...
class ParamParser(ParamParserInterface):
    ...
    def parse_model():
        ...
        if 'CNN' in mdl_cfg.ml_method_name :
            ml_model = eval(mdl_cfg.ml_method_name)(target_shape=y_size, **mdl_cfg_inner)
            ml_model.compile(loss=mdl_cfg.loss_calc_method,metrics=mdl_cfg.metrics, optimizer=optimizer, run_eagerly=True)
